...
def parse_comment_entries(comments_json: str) -> List[LedgerEntry]:
    """Parse comment entries from JSON file."""
    if not Path(comments_json).exists():
        return []
...
def load_entries(args: argparse.Namespace) -> List[LedgerEntry]:
    ...
    # Parse comments
    entries.extend(parse_comment_entries(args.comments_json))
...
def apply_xp_updates(
    ...
    for entry in valid_entries:
        ...
        reason = f"Backfill from ledger #{entry.pending_id} ({entry.source})"
        ...
        if dry_run:
            ...
            try:
                ...
                print(f"Applied {tier} XP to {entry.user}: {reason}")
            except subprocess.CalledProcessError as e:
                print(f"Error applying XP to {entry.user}: {e}")
...
def main() -> None:
    ...
    valid_entries = [e for e in entries if e.status != "voided"]
    print(f"Valid entries (non-voided): {len(valid_entries)}")
