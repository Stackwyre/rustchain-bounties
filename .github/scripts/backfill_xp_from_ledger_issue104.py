...
def parse_comment_entries(comment_body: str, source: str) -> List[LedgerEntry]:
    """Parse comment for payout entries via bullet blocks and table rows."""
    out: List[LedgerEntry] = []
...
def load_entries(args: argparse.Namespace) -> List[LedgerEntry]:
    ...
    if not args.comments_only:
        if Path(args.issue_json).exists():
            ...
            print(f"Loaded {len(body_entries)} entries from issue body")
    ...
    if Path(args.comments_json).exists():
        ...
        print(
            f"Loaded {len([e for e in all_entries if e.source.startswith('comment')])} entries from comments"
        )
...
def apply_xp_updates(
    ...
    for entry in entries:
        if entry.status == "voided":
            print(
                f"Skipping voided entry: {entry.user} {entry.amount} RTC (pending #{entry.pending_id})"
            )
            continue
        ...
        cmd = [
            ...
            "--local",
            "--reason",
            f"Backfill from issue #104 pending #{entry.pending_id}",
        ]
        ...
        if dry_run:
            ...
            print(
                f"Applying XP: {entry.user} +{tier} (from {entry.amount} RTC, pending #{entry.pending_id})"
            )
            try:
...
def main() -> None:
    ...
    print(f"\nSummary by user ({len(user_summary)} users):")
    for user, user_entries in sorted(user_summary.items()):
